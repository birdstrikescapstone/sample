...
function(input, output, session) {
  ...
  if (!is.null(qs$id)) {
    rv$schooldata <-
      query_db(sprintf(
        "SELECT * FROM data WHERE id = %d ORDER BY year",
        as.numeric(qs$id)
      )) %>%
      mutate(school.name = uncap(school.name),
             school.city = uncap(school.city))
  ...
  search_results <- eventReactive(input$search_button, {
    req(input$search)
    validate(need(
      nchar(input$search) >= 3,
      "Please enter at least three characters"
    ))
    
    results <-
      db %>% tbl("schoolnames") %>% arrange(school.name) %>% collect() %>%
      ...
      div(
        ...
        table(
          ...
          mapply(USE.NAMES = FALSE, SIMPLIFY = FALSE, function(id, name, city, state) {
            tr(td(strong(
              a(href = paste0("?id=", id), target = "_top", name)
            )),
            td(sprintf("%s, %s", city, state)))
  ...
  output$school_city <- renderText({
    paste(rv$schoolsummary$school.city,
          rv$schoolsummary$school.state,
          sep = ", ")
  })
  output$school_size <- renderText({
    paste(format(rv$schoolsummary$student.size, big.mark = ","),
  ...
  output$admit_rate <- renderText({
    rate <-
      tail(na.omit(rv$schooldata$admissions.admission_rate.overall),
           1)
    if (length(rate) == 0)
      return("No data")
  ...
  output$salary <- renderText({
    salary <-
      tail(na.omit(rv$schooldata$earnings.10_yrs_after_entry.median),
           1)
    if (length(salary) == 0)
      return("No data")
  ...
  output$cost_by_income <- renderTable({
    ...
    value_for_level <- function(lvl) {
      priv <-
        rv$schooldata[[paste0("cost.net_price.private.by_income_level.", lvl)]]
      pub <-
        rv$schooldata[[paste0("cost.net_price.public.by_income_level.", lvl)]]
  ...
  output$cost_by_year <- renderPlot({
    cost <-
      rv$schooldata %>% select(year, cost.avg_net_price.overall) %>%
    ...
    cost %>%
      ...
      geom_text(
        aes(label = paste0(
          "$", format(cost.avg_net_price.overall, big.mark = ",")
        )),
        vjust = -0.8,
        color = "white",
        size = 4.7,
    ...
    if (rv$app_mode == "search") {
      ...
      tags$style(
        ...
        sprintf(
          ...
          flickr_photo_url(
            flickr_photos_search_one(api_key, rv$schoolsummary$school.name)
          )
        ),
        "footer {display: block;}"
      )
